...
class FlowNet2(nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        if sz[2] % 64 == 0 and sz[3] % 64 == 0:
            img1 = img1.view(sz[0], sz[1], 1, sz[2], sz[3])
            img2 = img2.view(sz[0], sz[1], 1, sz[2], sz[3])
            inputs = torch.cat((img1, img2), dim=2)
        else:
            img1 = F.upsample(input=img1, size=(sz[2] - sz[2] % 64, sz[3] - sz[3] % 64), mode='bilinear',
                              align_corners=True)
            img2 = F.upsample(input=img2, size=(sz[2] - sz[2] % 64, sz[3] - sz[3] % 64), mode='bilinear',
                              align_corners=True)
            img1 = img1.view(sz[0], sz[1], 1, sz[2] - sz[2] % 64, sz[3] - sz[3] % 64)
            img2 = img2.view(sz[0], sz[1], 1, sz[2] - sz[2] % 64, sz[3] - sz[3] % 64)
            inputs = torch.cat((img1, img2), dim=2)
        ...
        if self.fp16:
            resampled_img1 = self.resample1(x[:, 3:, :, :].float(), flownetc_flow.float())
            resampled_img1 = resampled_img1.half()
        else:
        ...
        if self.fp16:
            resampled_img1 = self.resample2(x[:, 3:, :, :].float(), flownets1_flow.float())
            resampled_img1 = resampled_img1.half()
        else:
        ...
        if self.fp16:
            diff_flownets2_flow = self.resample4(x[:, 3:, :, :].float(), flownets2_flow.float())
            diff_flownets2_flow = diff_flownets2_flow.half()
        else:
        ...
        if self.fp16:
            diff_flownetsd_flow = self.resample3(x[:, 3:, :, :].float(), flownetsd_flow.float())
            diff_flownetsd_flow = diff_flownetsd_flow.half()
        else:
        ...
        if (torch.isnan(flownetfusion_flow).sum()):
        ...
        if sz[2] % 64 == 0 and sz[3] % 64 == 0:
            ...
            flownetfusion_flow = F.upsample(input=flownetfusion_flow, size=(sz[2], sz[3]), mode='bilinear',
                                            align_corners=True)
...
class FlowNet2S_gls(FlowNetS.FlowNetS):
    def __init__(self, args, batchNorm=False, div_flow=20, requires_grad=False):
        super(FlowNet2S_gls, self).__init__(args, input_channels=6, batchNorm=batchNorm)
        self.rgb_max = args.rgb_max
        self.div_flow = div_flow
        if not requires_grad:
            for param in self.parameters():
    ...
    def forward(self, img1, img2):
        ...
        if self.training:
            ...
            # return self.upsample1(flow2 * self.div_flow)
            flow_result=self.upsample1(flow2 * self.div_flow)
            if sz[2] % 64 == 0 and sz[3] % 64 == 0:
                flow_result = flow_result
            else:
                assert False, "wrong"
                flow_result = F.upsample(input=flow_result, size=(sz[2], sz[3]), mode='bilinear',
                                            align_corners=True)
            return flow_result
