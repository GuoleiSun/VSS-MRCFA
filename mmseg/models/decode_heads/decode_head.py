...
class BaseDecodeHead_clips(nn.Module, metaclass=ABCMeta):
    ...
    def losses(self, seg_logit, seg_label):
        ...
        # exit()
        if self.atten_loss:
            assert type(seg_logit)==tuple and len(seg_logit)==2
            atten=seg_logit[1]
        ...
        if self.hypercorre and self.cityscape:
            ...
            if self.self_ensemble2 and seg_logit.shape[1]==2*seg_label.shape[1]:
                ...
                # # only use the supervision from the last frame, do Not use the supervison from the reference frame
                # # print("here22")
                # assert seg_logit.shape[1]==2*seg_label.shape[1]
                # num_clips=seg_label.shape[1]
                # seg_logit_ori=seg_logit[:,num_clips-1:num_clips]
                # batch_size, _, _, h ,w=seg_logit_ori.shape
                # seg_logit_ori=seg_logit_ori.reshape(batch_size*(1),-1,h,w)
                # seg_logit_lastframe=seg_logit[:,num_clips:].reshape(batch_size*(num_clips),-1,h,w)
                ...
                # seg_label_ori=seg_label[:,-1]
                # seg_label_lastframe=seg_label[:,-1:].expand(batch_size,num_clips,1,h,w).reshape(batch_size*(num_clips),1,h,w)
            elif self.self_ensemble2 and seg_logit.shape[1]<2*seg_label.shape[1]:
                # assert seg_logit.shape[1]==2*seg_label.shape[1]
                print("here2333")
                num_target_preds=seg_logit.shape[1]-seg_label.shape[1]
                num_clips=seg_label.shape[1]
                seg_logit_ori=seg_logit[:,:num_clips]
                batch_size, _, _, h ,w=seg_logit_ori.shape
                seg_logit_ori=seg_logit_ori.reshape(batch_size*(num_clips),-1,h,w)
                seg_logit_lastframe=seg_logit[:,num_clips:].reshape(batch_size*(num_target_preds),-1,h,w)

                batch_size, num_clips, chan, h ,w=seg_label.shape
                assert chan==1
                seg_label_ori=seg_label.reshape(batch_size*(num_clips),1,h,w)
                seg_label_lastframe=seg_label[:,-1:].expand(batch_size,num_target_preds,1,h,w).reshape(batch_size*(num_target_preds),1,h,w)
            elif self.self_ensemble2 and seg_logit.shape[1]==2*seg_label.shape[1]+1:
                assert seg_logit.shape[1]==2*seg_label.shape[1]+1
                num_clips=seg_label.shape[1]
                seg_logit_ori=seg_logit[:,:num_clips]
                batch_size, _, _, h ,w=seg_logit_ori.shape
                seg_logit_ori=seg_logit_ori.reshape(batch_size*(num_clips),-1,h,w)
                seg_logit_lastframe=seg_logit[:,num_clips:].reshape(batch_size*(num_clips+1),-1,h,w)

                batch_size, num_clips, chan, h ,w=seg_label.shape
                assert chan==1
                seg_label_ori=seg_label.reshape(batch_size*(num_clips),1,h,w)
                seg_label_lastframe=seg_label[:,-1:].expand(batch_size,num_clips+1,1,h,w).reshape(batch_size*(num_clips+1),1,h,w)
            elif seg_logit.shape[1]==2*seg_label.shape[1]:
                num_clips=seg_label.shape[1]
                seg_logit_ori=seg_logit[:,:num_clips-1]
                batch_size, _, _, h ,w=seg_logit_ori.shape
                seg_logit_ori=seg_logit_ori.reshape(batch_size*(num_clips-1),-1,h,w)
                seg_logit_lastframe=seg_logit[:,num_clips-1:].reshape(batch_size*(num_clips+1),-1,h,w)

                batch_size, num_clips, chan, h ,w=seg_label.shape
                assert chan==1
                seg_label_ori=seg_label[:,:-1].reshape(batch_size*(num_clips-1),1,h,w)
                seg_label_lastframe=seg_label[:,-1:].expand(batch_size,num_clips+1,1,h,w).reshape(batch_size*(num_clips+1),1,h,w)
            elif seg_logit.shape[1]==2*seg_label.shape[1]+5:
                num_clips=seg_label.shape[1]
                seg_logit_ori=seg_logit[:,:num_clips-1]
                batch_size, _, _, h ,w=seg_logit_ori.shape
                seg_logit_ori=seg_logit_ori.reshape(batch_size*(num_clips-1),-1,h,w)
                seg_logit_lastframe=seg_logit[:,num_clips-1:].reshape(batch_size*(num_clips+6),-1,h,w)

                batch_size, num_clips, chan, h ,w=seg_label.shape
                assert chan==1
                seg_label_ori=seg_label[:,:-1].reshape(batch_size*(num_clips-1),1,h,w)
                seg_label_lastframe=seg_label[:,-1:].expand(batch_size,num_clips+6,1,h,w).reshape(batch_size*(num_clips+6),1,h,w)

                preds_logits=F.softmax(seg_logit[:,-6:],2)
                preds_ave=preds_logits.mean(1, keepdim=True)
                nega_corre=(preds_logits-preds_ave)
                # print(nega_corre.shape,preds_logits.shape)
                loss['loss_nega_corre']=-self.loss_nega_corre(nega_corre,torch.zeros_like(nega_corre)) 
        else:
            # print("here3")
        ...
        if self.atten_loss:
            logit_size = atten.shape[2:4]
            ideal_affinity_matrix=self._construct_ideal_affinity_matrix(seg_label, logit_size)
            shape0,shape1,shape2,shape3,shape4,shape5=atten.shape
            atten=atten.reshape(shape0*shape1, shape2*shape3, shape4*shape5)
            assert atten.shape==ideal_affinity_matrix.shape
            atten=torch.sigmoid(atten)
            # print(atten.shape, ideal_affinity_matrix.shape, torch.unique(ideal_affinity_matrix))
            # print(atten.min(),atten.max(), ideal_affinity_matrix.sum())
            prior_loss = self.loss_prior_decode(atten, ideal_affinity_matrix)
            loss['loss_prior'] = prior_loss

        # if self.self_ensemble2:
        #     assert seg_logit_lastframe.dim()==4
        #     batch_size, num_clips=seg_label.shape[:2]
        #     h,w=seg_label.shape[3:]
        #     seg_logit_lastframe=seg_logit_lastframe.reshape(batch_size,num_clips,-1,h,w)
        #     ensemble_pred=F.softmax(seg_logit_lastframe,dim=2).sum(1)
        #     ensemble_pred = torch.log(torch.clamp(ensemble_pred, min=1.e-15))
        #     loss['loss_ensemble'] = 0.0*F.cross_entropy(ensemble_pred, seg_label[:,-1].squeeze(1), ignore_index=self.ignore_index)
